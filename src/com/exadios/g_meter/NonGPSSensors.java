...
public class NonGPSSensors implements SensorEventListener, Runnable {
  ...
  public void onSensorChanged(SensorEvent event) {
    ...
    try {
      ...
      case Sensor.TYPE_ACCELEROMETER:
        setAcceleration(event.timestamp, 
                        event.values[0],
                        event.values[1],
                        event.values[2]);
        break;
      case Sensor.TYPE_GYROSCOPE:
        setRotation(event.timestamp, 
                    event.values[0],
                    event.values[1],
                    event.values[2]);
        break;
      case Sensor.TYPE_MAGNETIC_FIELD:
        setMagneticField(event.timestamp,
                         event.values[0],
                         event.values[1],
                         event.values[2]);
        break;
      case Sensor.TYPE_PRESSURE:
        setBarometricPressure(event.timestamp,
                              event.values[0],
  ...
  private native void setAcceleration(long time,
                                      float ddx,
                                      float ddy,
                                      float ddz);
  private native void setRotation(long time,
                                  float dtheta_x,
                                  float dtheta_y,
                                  float dtheta_z);
  private native void setMagneticField(long time,
                                       float h_x,
                                       float h_y,
                                       float h_z);
  private native void setBarometricPressure(long time,
                                            float pressure,
